...
const checkNicknameSchema = () => {
  ...
  isNicknameValid = checkSchema(
    nicknameInput,
    [{ condition: () => !!nickname, errorMessage: "닉네임을 입력해 주세요" }],
    nicknameErrorElement
  );
  updateButtonState();
};
// 비밀번호 유효성 검사
...
const checkPasswordRepeatSchema = () => {
  ...
  const passwordRepeatErrorElement = document.getElementById(
    "passwordRepeatError"
  );
  isPasswordRepeatValid = checkSchema(
    ...
    [
      {
        condition: () => !!passwordRepeat,
        errorMessage: "비밀번호 확인을 입력해 주세요",
      },
      {
        condition: () => passwordRepeat === passwordInput.value,
...
const togglePasswordVisibility = (event) => {
  ...
  eyeIcon.src = isPasswordType ? eyeVisible : eyeInvisible;
  eyeIcon.alt = isPasswordType
    ? "비밀번호 표시 상태 아이콘"
    : "비밀번호 숨김 상태 아이콘";

  button.setAttribute(
    "aria-label",
    isPasswordType ? "비밀번호 숨기기" : "비밀번호 보기"
  );
};
